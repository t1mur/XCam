...
namespace XCamera
{
    public partial class MultiImageViewer2 : UserControl
    {
        private bool picturePress = false;
        ...
        public static Pen cursorPen = new Pen(Color.Green);
        ...
        public void setPictureBoxSize(Size pbSize)
        {
            ...
            this.Size = new Size(Math.Max(385, pbSize.Width), 97+ 44 + pbSize.Height);
        ...
        private void displayNCounts()
        {
            ...
            labelNCount.Text = (int)defaultNCount +"";
        }
        ...
        private void redraw(Graphics graphics)
        {
            ...
            graphics.DrawLine(cursorPen, new Point(0, lastCursor.Y), new Point(pictureBox.Size.Width, lastCursor.Y));
            graphics.DrawLine(cursorPen, new Point(lastCursor.X, 0), new Point(lastCursor.X, pictureBox.Size.Height));
        }
        private void redraw()
        {
            redraw(graphics);
        ...
        private void displayPixelValue(int y, int x)
        {
            ...
            for (int i = 0; i < NDisplayPixelValue; i++)
            {
                ...
                result = NumToString(val, 5);
                if (i < NDisplayPixelValue-1) result = result + ", ";
        ...
        private void pictureBox_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (images != null)
                if (images.Count > currentImageID)
                {
                    ...
                    xLabel.Text = "x=" + NumToString(p.X,4);
                    yLabel.Text = "y=" + NumToString(p.Y,4);
                }
            if (isDraging)
            {
        ...
        private void crossSectionToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            CrossSectionForm csf = new CrossSectionForm();
            csf.displayCrossSection(image, image.fromRelativeToAbsolute(defaultRectangle));
            csf.Show();
        }
        ...
        private void displayCursorValues()
        {
            Point cursor = images[0].fromRelativeToAbsolutePoint(lastCursor);
            string intensity = NumToString(images[0].data[cursor.Y, cursor.X], 5);
            cursorLabel.Text = "Cursor: x=" + NumToString(cursor.X, 4) + 
                                     ", y=" + NumToString(cursor.Y, 4) + 
                                     ", I=" + intensity;
        }

        public static string NumToString(Single n, int digit)
        {
            string s = "" + n;
            if (s.Length > digit) s = s.Substring(0, digit);
        ...
        {
            Point oldCursor = lastCursor;
            if (e.KeyCode == Keys.Right) lastCursor.X++;
            if (e.KeyCode == Keys.Left) lastCursor.X--;
            if (e.KeyCode == Keys.Up) lastCursor.Y--;
            if (e.KeyCode == Keys.Down) lastCursor.Y++;
            if ((images != null) && (images.Count > 0) && (images[0] != null) && (lastCursor != oldCursor))
            {
                displayCursorValues();
                redraw();
            }
        ...
        {
            if (picturePress) pictureBox.Focus();
            picturePress = false;
